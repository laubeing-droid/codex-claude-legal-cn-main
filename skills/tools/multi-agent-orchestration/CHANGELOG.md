...
- 明确 Claude Code 官方 Agent Teams 的状态源在 `用户技能目录/teams/<team>/` 与 `用户技能目录/tasks/<team>/`，不要在项目内自造 `.claude/teams/` 冒充官方 team。
...
### Changed
- **[重大] tmux 模式统一使用 Agent Teams 文件通信协议**：tmux 仅作为进程管理层，通信层复用 `用户技能目录/teams/` 的 inbox + tasks 机制
