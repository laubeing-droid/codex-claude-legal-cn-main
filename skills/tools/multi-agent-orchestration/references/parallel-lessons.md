...
Claude Code 官方 Agent Teams 使用用户目录下的团队和任务状态：`用户技能目录/teams/{name}/config.json`、`用户技能目录/teams/{name}/inboxes/` 和 `用户技能目录/tasks/{name}/`。这是官方 team 状态源；不要在项目内新建 `.claude/teams/` 来冒充官方 team。
...
**tmux/custom worker**：默认不写 `用户技能目录/teams/`，而是把 PM checkpoint 写在当前 worktree 的 `.claude/agent-sessions/{session}/`。只有明确接入 Agent Teams inbox 时，才额外写 health_report。
