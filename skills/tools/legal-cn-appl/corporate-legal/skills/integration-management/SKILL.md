...
存放于 `本地配置目录 `deal-context.md` 获取交易代码、目标公司名称、交割日期和交易负责人。从 `closing-checklist.yaml`（如存在）继承任何交割后项目。
...
读取 `本地配置目录 deal-context.md 不存在，写入。

读取 `本地配置目录 Day 1 或 Day 30 工作计划项（从交割检查表继承状态）。
...
> - 交割检查表——如由并购冷启动技能生成，我将自动从 `本地配置目录 继承
