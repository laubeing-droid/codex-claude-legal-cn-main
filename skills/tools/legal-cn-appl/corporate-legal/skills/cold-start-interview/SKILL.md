...
6. 写入 `本地配置目录 `--new-deal`，写入 `本地配置目录
...
如果旧缓存路径 ` 存在 CLAUDE.md 但配置路径不存在，在继续前将其复制到配置路径。
...
- 如果你能测试连接（调用一个简单的 联网检索 工具如列表或搜索），仅在成功响应时报告 ✓。
- 如果你无法测试（无法从此处探测），报告 ⚪ "已配置但未验证——打开你的 联网检索 设置确认"附一行如何确认。
- 绝不基于单独配置报告 ✓。

然后按此形式报告发现：

> - ✓ [集成] — 已连接（已验证）
> - ⚪ [集成] — 已配置但未验证。打开 联网检索 设置确认。
...
写入 `本地配置目录 `deal-context.md`（此笔交易），deal-context.md 在冲突时优先。
