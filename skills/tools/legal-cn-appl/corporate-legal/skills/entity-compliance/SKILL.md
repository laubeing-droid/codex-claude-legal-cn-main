...
1. 加载 `本地配置目录 → `## 主体管理`（主体清单、注册地、工商登记代办机构）。
2. 按以下标志路由到正确的模式：
...
> 本追踪器按每个主体记录的设立地或经营地计算截止日。申报规则、截止日机制和费用结构在不同注册地之间存在实质性差异。如果主体的实际业务范围与 `本地配置目录 中记录的不同（未披露的外地经营备案、已注销主体、注册地迁移、由本地代理机构管理的跨省级申报），输出可能不完全适用——与工商登记代办机构或该辖区的当地律师确认。
...
读取 `本地配置目录 → `## 主体管理` → 主体清单。如果主体清单已填充（来自冷启动时的组织架构图上传），直接使用。如果未填充，请用户运行冷启动模块或提供主体清单。
...
**在指示或确认一项申报前：** 读取 `本地配置目录 中的 `## 使用者`。如果角色为**非法务人员**：
...
**经营备案缺口：**
- 基于 `本地配置目录 的主体清单：公司的业务足迹中是否有省份（办事处、员工）主体未办理经营备案？这需要律师确认业务存在——Claude 可以提出问题但不能独立判断业务存在。
...
潜在缺口
 ...
 [足迹在追踪器中未显示为已备案的省份列表，来自 `本地配置目录
 关联方交易协议订立：[来自 `本地配置目录 的状态]
