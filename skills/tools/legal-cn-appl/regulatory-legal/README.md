...
监测监管法规动态，对比新法规与政策库的差异，发现合规差距。学习你的重要性阈值，不会对每条部门动态都报警。通过案例库法律检索和部委网站进行法规监测。
...
负责人通知（差距分配、到期提醒、征求意见稿预警）需要环境中配置消息类 检索服务。没有时，差距追踪器和征求意见追踪器仍可工作——只是通知不会推送，技能会在状态报告中标注未推送事项。

## 如何持续学习

你的实务画像位于 `本地配置目录 不是静态的——随着你使用插件不断改进。技能会告知你输出何时使用了应调整的默认值。`reg-change-monitor` 代理监测法规动态并对照你的政策库标注变化。你可以重新运行设置、直接编辑文件或告知技能记录新立场。
