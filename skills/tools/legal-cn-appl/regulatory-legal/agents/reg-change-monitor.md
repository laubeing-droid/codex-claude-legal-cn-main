...
description: >
 ...
 Runs per the cadence in 本地配置目录 Filters by materiality threshold so the
...
Per `本地配置目录 → Feed configuration → Check cadence. Default weekly; daily if the regulatory environment is active.

## What it does

1. Read `本地配置目录 → watchlist, materiality threshold.
