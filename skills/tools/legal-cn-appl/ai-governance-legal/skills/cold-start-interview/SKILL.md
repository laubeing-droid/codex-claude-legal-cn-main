...
1. 检查 `本地配置目录 是否已存在。如果存在且用户未传递 `--redo` → "AI治理实践配置已存在于 [路径]。使用 `--redo` 重新运行。"
2. 运行以下访谈。一次进行一个部分。
3. 选项后附 `(✓)` 标注推荐默认值。
...
☐ 不利用AI生成或传播违法和不良信息
☐ 不利用算法实施垄断或不正当竞争行为（《中华人民共和国反垄断法》第9条 + 《算法推荐管理规定》第15条 `[法条原文]`）
☐ 其他：[填写]
...
| 数据跨境传输 | A. 数据不得出境 (✓) / B. 经安全评估后可出境 / C. 无固定立场（需符合《数据安全法》+《中华人民共和国个人信息保护法》第三章 `[法条原文]`） |
