...
- `本地配置目录 —— 风险校准（分流门槛、重要性、和解阶梯）、执业背景（相关方、外聘律师库）。
- `本地配置目录 —— 确认代号唯一性。
...
在进一步操作前，按 `本地配置目录 → 利益冲突审查运行冲突检索步骤。
 ...
 **路径1 —— 现在运行冲突检索。** 暂停本登记。按 `本地配置目录 的冲突审查完成。返回时带 `status: cleared` 或 `status: waived` 附理由。

 **路径2 —— 标注待定，附负责人+截止日期。** 仅在 `本地配置目录 的冲突审查声明允许平行登记时可用。记录：谁在运行冲突检索、预计何时返回、检查哪些主体。登记继续；案件行携带 `conflicts.status: pending`；`/portfolio-status` 每次运行均标注；`/matter-update` 重复提示直至解决。
