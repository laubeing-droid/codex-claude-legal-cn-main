...
1. 读取提供的来函文件。
2. 加载 `本地配置目录 用于案件组合交叉检索。
3. 加载 `本地配置目录 → 风险校准、执业背景、律师函实务惯例。
4. 按以下工作流操作。
...
- 来函文件（用户提供路径或在会话中发送）
- `本地配置目录 —— 扫描关联案件（相同对方、通过实体关系关联的对方、或同类案件类型+近期日期）
- `本地配置目录 → 风险校准（用于实质理由评估）、执业背景（发函方是否为经常性对手？）、律师函实务惯例（事务所语调和回复默认策略）
...
- **筹码与压力** —— 对方是否真的准备起诉？是否有诉讼能力？是否属于 `本地配置目录 中记录的常发性对手？
...
**来源标注。** 为分流意见中的每个引用——包括发函方援引的法律依据、我方回复方案的理由、以及实质理由评估中调取的研究——标注来源：`[案例库检索]`用于通过检索连接器获取的引用；`[联网检索——需复核]`用于联网搜索引用；`[模型知识——需验证]`用于模型知识回忆的引用；`[用户提供]`用于来函中援引的法规。标注`需验证`的引用具有较高的编造风险，应首先核验。不得删除或压缩标签。
