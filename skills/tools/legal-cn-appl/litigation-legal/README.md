...
**请先连接检索工具——引用保护机制依赖于此。** 无检索工具时，每条引用均标注 `[需核实]`，审查备注记录来源未经验证。通过 **yuan dian 联网检索**（案例语义检索、法规检索）、**聚法案例**或**人民法院案例库**检索获得的引用，标注来源并可追溯。来自模型知识或联网搜索的引用标注 `[需核实]` 或 `[需核实-精确引用]`，应在信赖前对照一手来源核验。插件对引用分层标注，使你的核实时间集中在最重要的地方。
