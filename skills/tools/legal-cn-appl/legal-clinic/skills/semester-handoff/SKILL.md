...
1. 加载 `本地配置目录 → 诊所画像、学期日期、指导风格。
2. 加载 `本地配置目录 和按案件的 `本地配置目录
3. 使用以下工作流。
...
- `本地配置目录 → 诊所画像、学期、实践领域、指导风格
- `本地配置目录 → 所有活跃截止日期，按案件分组
- `本地配置目录 → 沟通历史
- 诊所维护的案件文件 / 接待摘要
- 学生名册——谁在交接中负责什么
...
- 拉取所有活跃案件（来自接待记录 + `本地配置目录 案件编号 + client-comms 文件夹）
...
*从 `本地配置目录 拉取。新接手学生的第一项工作是确认这些准确并已认领。*
...
结案或将案件移交给新学生是一项具有法律后果的行为。门控是 `本地配置目录 中 `## 指导风格` 描述的指导工作流程，由确认持证指导律师拥有设置的 Part 0 身份检查强化。无论选择何种指导风格，结案备忘录在案件中标记为已关闭前始终获得指导老师签字。
...
审查后，交接备忘录保存在 `本地配置目录 `/ramp` 运行中读取它们——`/ramp` 应为新学生分配的案件浮现备忘录。

## 联动

- **`/ramp`：** 下学期初，读取 `本地配置目录 并为每个新学生接手的案件浮现按案件备忘录。
