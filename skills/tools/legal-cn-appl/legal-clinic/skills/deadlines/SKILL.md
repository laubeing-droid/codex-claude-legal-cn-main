...
1. 加载 `本地配置目录 → 管辖地、实践领域、预警天数节奏。
2. 使用以下工作流。
3. 按标志路由：
 - `--add`：捕获案件、类型、描述、截止日期、来源、负责人。写入 `本地配置目录
...
- `本地配置目录 → 管辖地、实践领域、截止日期预警天数（默认14/7/3/1）、指导律师
- `本地配置目录 ——分类台账
...
- **`/status`：** status 技能读取相关案件的 `本地配置目录 并将其活跃截止日期纳入输出。
