...
1. 加载 `本地配置目录 → 通俗语言标准、指导风格、诊所联系信息。
2. 使用以下模板和工作流。
3. 匹配类型到模板。通俗语言检查。
4. 输出附 AI 辅助标签、指导路由。
...
向当事人发送信函是一项具有法律后果的行为。本插件的门控是 `本地配置目录 中 `## 指导风格` 描述的指导工作流程，由确认持证指导律师拥有诊所设置的 Part 0 身份检查强化。该门控仍有效：每封信函在离开诊所前均需通过审查。

在发送上述任何信函前，确认：

1. 草稿已按 `本地配置目录 中的指导协议审查（队列 / 标记 / 较轻触）。
...
按 `本地配置目录 标准。短句。无法律术语。强制阅读水平目标。如果上述模板中包含当事人可能不理解的法律术语，首次出现时解释："我们已提交了'答辩状'——这是向法院说明您对案件看法的文件。"
