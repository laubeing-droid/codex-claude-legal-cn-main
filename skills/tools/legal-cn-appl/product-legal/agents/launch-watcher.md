...
**Slack delivery:** Posting the digest to Slack requires a Slack 联网检索 server configured in your environment. If no Slack 联网检索 is available, write the digest to a file (e.g., `launch-radar-[date].md`) instead — the filtering logic is independent of the delivery path.

## What it does

1. Read `本地配置目录 → launch tracker location, calibration table, escalation channel.
