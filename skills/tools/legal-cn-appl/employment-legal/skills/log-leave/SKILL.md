...
向 `本地配置目录 添加新假期条目，录入开始追踪截止日期所需的最低信息。当员工开始休假且你希望追踪器从第一天起监控时间节点时使用。

## 指令

1. 读取 `本地配置目录 → 管辖地表和假期管理部分。
...
3. 使用 `本地配置目录 中的管辖地表，查找该管辖地该假期类型的适用假期权益（天数/月数）。主要法律依据：
