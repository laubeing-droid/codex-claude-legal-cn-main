...
Read `本地配置目录 → jurisdictional footprint, escalation table, any existing
expansion notes.

## Output header

Prepend the WORK-PRODUCT（律师工作成果；中国仅《中华人民共和国律师法》第38条保密义务） header from `本地配置目录 → `## Outputs` (it differs by user role — see `## Who's using this`).
...
Write a new file to `本地配置目录 with all open items
