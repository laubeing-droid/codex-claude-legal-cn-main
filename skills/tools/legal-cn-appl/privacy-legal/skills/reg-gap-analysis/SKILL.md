...
1. 加载 `本地配置目录 → 个人信息处理规则承诺、监管覆盖范围、DSAR系统。
2. 执行以下工作流。
...
本技能将新要求与你当前的实际情况（按 `本地配置目录 → 个人信息处理规则承诺 + PIA 中记录的实际做法）进行差异对比，并产出一份带整改计划的差距清单。
...
> 本输出中的引用由 AI 模型生成，尚未与主源核对。依赖任何法规、指引或执法行动前，请对照法律检索工具（如 案例库 联网检索 或发布机关官网）核查准确性和现行效力。AI 生成的引用可能存在编造或引用错误。每条引用上的来源标签（如 `[联网检索 — 需复核]`）显示其来源；带 `验证` 的标签造假风险更高，应优先核对。
