...
1. 检查 `本地配置目录 — 如已填充且无 `--redo`，覆盖前先确认。
2. 运行以下访谈工作流。
...
重新运行集成可用性检查（文档存储、即时通讯、定时任务）并更新 `本地配置目录 中的 `## 可用集成`。不重新访谈。当你连接或断开一个 联网检索 并希望插件在不重新运行完整设置的情况下感知时使用。
...
了解*这个*隐私团队如何工作——什么法规实际适用于他们，他们在 DPA 中愿意/不愿意接受什么，一份好的 PIA 在这里和别处有什么不同。写入 `本地配置目录
...
如果旧的缓存路径 ` 中存在 CLAUDE.md 但不在配置路径，复制过来。
...
- **暂停和恢复。** 预先告诉用户："如果你需要停止，说'暂停'（或'停'，或'让我回头再来'）我会保存你的进度。稍后再次运行 `/privacy-legal:cold-start-interview` 我会从你停下的地方继续。"当用户暂停时，向 `本地配置目录 写入部分配置，并在顶部附 `<!-- 设置暂停于：[章节名称] — 运行 /privacy-legal:cold-start-interview 以恢复 -->` 注释，未填写的字段用 `[待处理]` 标记（区别于 `[占位符]`）。当设置重新运行并发现暂停的配置时，问候用户："欢迎回来。你暂停于[章节]。你此前的回答已保存。从停下的地方继续，还是从头开始？"不要重新提问已回答的问题。
...
- 如果你能测试连接（调用简单的 联网检索 工具如列出或搜索），仅在成功响应时报告 ✓。
- 如果你不能测试（无法从此处探测），报告 ⚪"已配置但未验证——打开你的联网检索设置以确认"并附单一行的操作方法。
- 绝不基于单独的配置报告 ✓。

然后以此形式报告发现：

> - ✓ [集成] — 已连接（已测试）
> - ⚪ [集成] — 已配置但未验证。打开你的联网检索设置以确认。
