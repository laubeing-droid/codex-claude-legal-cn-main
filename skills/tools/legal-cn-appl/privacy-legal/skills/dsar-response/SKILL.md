...
1. 加载 `本地配置目录 → DSAR 流程（系统清单、验证方式、SLA）。
2. 执行以下工作流。
...
本分析假定你的配置中指定的法域范围。隐私规则、回复期限和合法性基础因法域而异（个保法 vs. （注：中国适用《个人信息保护法》/PIPL） ）。如果个人信息主体、处理活动或处理者位于不同于配置的法域，本分析可能不直接适用。

## 加载流程

读取 `本地配置目录 → `## DSAR 流程`。该节包含：
- 系统清单（用户数据所在的每个位置）
- 身份验证方式
- 回复 SLA
- 谁处理常规请求 vs. 谁处理升级请求
...
**在将任一函件发送给个人信息主体之前：** 读取 `本地配置目录 中的 `## 谁在使用`。如果角色为非律师：
...
> **注意：** 两份 DSAR 函件均为对外发送给个人信息主体的文件。**勿**在两份函件上包含 `本地配置目录 `## 输出` 中的工作成果抬头。函件附随的内部笔记、日志和豁免分析是律师工作成果——将这些分开保存并冠以工作成果抬头。
...
如果 `本地配置目录 → `## DSAR 流程` 记录了一个比法定期限更严格的内部 SLA，使用内部 SLA 并注明法定底线。
