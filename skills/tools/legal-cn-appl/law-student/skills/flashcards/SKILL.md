...
1. 加载 `本地配置目录 → 当前课程、薄弱科目、大纲位置。
2. 应用以下框架。
3. 按标志路由：
 - `--generate`：从来源（大纲路径、笔记、教材）按卡片编写规则构建卡片。写入 `本地配置目录
...
- 如果无 `study-plan.yaml`，改为写入 `本地配置目录

### `--generate` — 创建卡片

**输入：**
- 科目（课程名称或主题）
- 来源（大纲路径、笔记，或"使用我在 本地配置目录 中的现有大纲"）
...
**引用核验。** 当卡片是从我的知识而非你粘贴的来源生成时，背面的规则和任何引用的案例/法条由 AI 模型生成且未经核实。在记下一张卡片之前，对照你的大纲、教材或研究工具（法律数据库、法信、中国裁判文书网）核实。一张训练到精通的错误卡片比没有卡片更糟糕。
