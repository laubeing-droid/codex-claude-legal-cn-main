...
description: >
 ...
 within a rolling 12-month window (configurable in `本地配置目录).
...
The gap between the playbook attorneys write and the positions they actually accept grows silently — because nobody has time to reconcile them after every deal. This agent watches the deviation log, detects when a position is being overridden consistently, and proposes a specific update to `本地配置目录 The attorney approves or rejects. The playbook stays alive.
...
Both values are configurable in `本地配置目录 under `## Playbook monitor settings`:
...
If these fields are absent from `本地配置目录 use the defaults above.

## What it does

### Step 1 — Read the practice profile and log

1. Read `本地配置目录 in full. Extract:
...
If no clause crosses the threshold: log the check to `本地配置目录 and stop. Do not notify the attorney.
...
2. **Current playbook language** (exact text from `本地配置目录)
...
Write all proposals to `本地配置目录 Overwrite any existing file — stale unreviewed proposals are replaced, not accumulated.
...
Notify the attorney via the destination in `本地配置目录
...
1. Read `本地配置目录 If file doesn't exist or is empty: *"No pending proposals. Playbook is up to date."* Stop.
...
[A] Accept — apply proposed language to `本地配置目录
...
5. **Reject / Defer:** log to `本地配置目录 with reason if given. Do not modify `本地配置目录 A rejected proposal is not re-raised until a new pattern emerges after the rejection date.

6. After all proposals resolved, show summary:

```
Review complete.
[N] accepted and applied to `本地配置目录
[N] rejected
[N] deferred to next cycle
[N] edited and applied

`本地配置目录 last updated: [timestamp]
Next playbook check: after [N] more deals are logged
```

7. Archive: rename `本地配置目录 to `本地配置目录 The active file is now clear.

## What this agent does NOT do

- Modify `本地配置目录 without explicit per-change attorney confirmation
