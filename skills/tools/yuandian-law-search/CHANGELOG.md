...
### 文档完善
- SKILL.md 和 README.md 改为推荐使用 `scripts/yd-run`，降低 Codex 网络沙箱、PATH 漂移和代理环境变量对案例库检索的影响。
...
### 新增
- 适配 24 个案例库开放平台新接口（从 11 个扩展至 35 个）
- 新增 5 个子命令：
...
- 排查案例库开放平台（2026-05-10）：通过 Playwright 浏览器实际访问接口广场，发现平台从 11 个 API 扩展到了 35 个，新增 24 个未适配接口（1 个幻觉检测 + 23 个企业信息），已记录到 MANIFEST.json，待后续适配
...
### 改进
- skill name 从 `yd-law-search` 改为 `案例库-law-search`，提升辨识度
- 目录同步重命名为 `案例库-law-search`
- 标题从"案例库法条检索"改为"案例库法条与案例检索"，准确反映 API 覆盖范围
- 许可证从 MIT 改为 MIT
...
### 设计缘由
- 案例库法条检索 API 提供了法律条文和案例的语义/关键词检索能力，适合封装为 Skill 供法律分析场景使用。
