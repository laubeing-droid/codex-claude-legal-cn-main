# 案例库法条与案例检索 (案例库-law-search)

通过 [案例库开放平台](https://open.chineselaw.com) 检索中国法律法规条文和案例，为法律分析和研究提供数据支撑。
...
`scripts/yd-run` 会用干净环境启动 Python，避免 Codex 进程环境、代理变量或 PATH 漂移影响案例库接口访问。网络排查可先运行：
...
案例库开放平台提供 11 个 API 端点，覆盖法条、案例、法规、企业四个领域。**每次 API 调用消耗 10 积分**，这意味着一个"检索 5 个案例并逐一查看详情"的简单场景，实际消耗为 10 + 5×10 = 60 积分。
...
| v0.2.0 | 2026-04-05 | 改名 `案例库-law-search`，MIT 许可证，新增注册引导 |
